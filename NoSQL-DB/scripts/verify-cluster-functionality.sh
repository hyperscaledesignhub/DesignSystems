...
# Comprehensive Cluster Functionality Verification Script
# Tests: Persistence, Replication, Fault Tolerance, Consistency
...
echo -e "${BLUE}🔍 Cluster Functionality Verification${NC}"
echo "======================================"
...
echo "Restarting node2 to test recovery..."
SEED_NODE_ID=db-node-2 python node.py &
NODE2_PID=$!
echo "Node2 restarted with PID: $NODE2_PID"
...
SEED_NODE_ID=db-node-3 python node.py &
PID_NODE3=$!
...
# Optional: Keep script running to maintain cluster
echo -e "\n${YELLOW}Cluster is running. Press Ctrl+C to stop.${NC}"
echo "PIDs: Node1: $(pgrep -f 'SEED_NODE_ID=db-node-1'), Node2: $PID_NODE2, Node3: $PID_NODE3"

# Cleanup function
cleanup() {
    echo -e "\n${YELLOW}Stopping verification script...${NC}"
    echo "Note: Cluster nodes are still running. Use 'pkill -f node.py' to stop them."
    exit 0
}
