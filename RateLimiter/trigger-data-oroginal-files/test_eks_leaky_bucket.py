...
CAPACITY = 5  # Maximum requests in the bucket (matching config.yaml)
LEAK_RATE = 2.0  # Requests leaked per second (matching config.yaml)
...
def test_leaky_bucket_rate_limiting():
    """Test rate limiting behavior of the leaky bucket endpoint."""
    client_id = f"test_client_{int(time.time())}"
    responses = []
    
    logger.info(f"\nTesting leaky bucket rate limiting for client {client_id}")
    logger.info(f"Bucket capacity: {CAPACITY}")
    logger.info(f"Leak rate: {LEAK_RATE} requests/second")
    ...
    for i in range(CAPACITY):
        ...
        if response["data"]:
            logger.info(f"Remaining requests: {response['data']['remaining']}")
    ...
    # Phase 3: Wait for bucket to leak and verify new requests succeed
    leak_wait_time = 1.0 / LEAK_RATE  # Time for one request to leak
    logger.info(f"\nPhase 3: Waiting {leak_wait_time} seconds for one request to leak")
    time.sleep(leak_wait_time + 0.1)  # Add small buffer for safety
    
    logger.info("Sending new request after leak")
    response = make_request(client_id)
    responses.append(response)
    logger.info(f"Request after leak: Status={response['status_code']}, Response={response['data'] if response['data'] else response['error']}")
    assert response["status_code"] == 200, "Request after leak should succeed"
    ...
    logger.info("All assertions passed - leaky bucket rate limiting is working as expected!")

if __name__ == "__main__":
    test_leaky_bucket_rate_limiting() 