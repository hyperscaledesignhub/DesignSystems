...
import { Briefcase, TrendingUp, TrendingDown, DollarSign, PieChart as PieIcon, Lock, Plus, AlertCircle } from 'lucide-react';
...
const Portfolio = () => {
  ...
  const fetchPortfolioData = async () => {
    try {
      ...
      // Try to get wallet balance first
      let balance = { available_balance: 0, blocked_balance: 0 };
      try {
        const balanceRes = await api.get('/wallet/balance');
        balance = balanceRes.data;
      } catch (error) {
        // Wallet might not exist, that's OK
        console.log('Wallet not found, will create on first deposit');
        setNoWallet(true);
      }
      
      const [positionsRes, tradesRes] = await Promise.all([
        api.get('/reports/positions').catch(() => ({ data: { positions: [] } })),
        api.get('/reports/trades?limit=50').catch(() => ({ data: { trades: [] } }))
      ]);
  ...
  const addFunds = async () => {
    ...
    try {
      ...
      setNoWallet(false);  // Clear the no wallet warning
      fetchPortfolioData();
      setShowAddFunds(false);
      setFundsAmount('');
      alert('Funds added successfully! Your wallet has been created if it didn\'t exist.');
    } catch (error) {
      console.error('Failed to add funds:', error);
      // Check if it's a specific error message
      const errorMessage = error.response?.data?.detail || 'Failed to add funds. Please try again.';
      alert(errorMessage);
    }
  };
  ...
  return (
    <div className="portfolio">
      ...
      {noWallet && (
        <div className="wallet-alert" style={{ 
          background: '#fff4e6', 
          ...
          <AlertCircle size={20} color="#ff8f00" />
          <span>No wallet found. Click "Add Funds" to create your wallet and add initial funds.</span>
        </div>
      )}
