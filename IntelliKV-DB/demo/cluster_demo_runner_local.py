...
class ClusterDemoRunner:
    ...
    def run_demo(self, demo_name: str, use_existing: bool = False) -> bool:
        """Run a specific demo with professional testing practices"""
        ...
        if demo_name not in demo_files:
            error_msg = f"Unknown demo: {demo_name}"
            print(f"❌ {error_msg}")
        ...
        if not os.path.exists(demo_path):
            error_msg = f"Demo file not found: {demo_path}"
            print(f"❌ {error_msg}")
            if self.logger:
                self.logger.error(error_msg)
            return False
        ...
        try:
            ...
            # Set config file path relative to demo directory
            config_relative_path = os.path.relpath(self.config_file, demo_cwd)
            env['CONFIG_FILE'] = config_relative_path
            ...
            # Add distributed path and lib path to PYTHONPATH for local imports
            distributed_path = os.path.join(os.path.dirname(demo_cwd), 'distributed')
            lib_path = os.path.join(distributed_path, 'lib')
            if 'PYTHONPATH' in env:
                env['PYTHONPATH'] = f"{distributed_path}:{lib_path}:{env['PYTHONPATH']}"
            else:
                env['PYTHONPATH'] = f"{distributed_path}:{lib_path}"
            ...
            if success:
                ...
                if self.logger:
                    self.logger.info(f"Demo {demo_name} completed successfully in {duration:.2f}s")
            else:
                print(f"❌ Demo {demo_name} failed after {duration:.2f}s")
                if self.logger:
                    self.logger.error(f"Demo {demo_name} failed after {duration:.2f}s")
            ...
            error_msg = f"Error running demo {demo_name}: {e}"
            print(f"❌ {error_msg}")
            if self.logger:
                self.logger.error(error_msg)
    ...
    def run(self, demo_name: str, use_existing: bool = False) -> bool:
        ...
        try:
            ...
            if use_existing:
                nodes = self.get_existing_nodes()
                if not nodes:
                    print("❌ No existing nodes found in config")
                    return False
                
                if not self.verify_existing_cluster(nodes):
...
EXAMPLES:
  ...
  When not specified, the runner will create and manage new nodes."""
    ...
    # Check if demo is provided when not using --list-demos
    if not args.demo:
        print("❌ Demo name is required when not using --list-demos")
        print("Use --list-demos to see available demos")
        sys.exit(1)
